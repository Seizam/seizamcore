	...
	private function _checkTpeParams($aConstants) {
		...
		for ($i = 0; $i < count($aConstants); $i++)
			if (!defined($aConstants[$i]))
				die ("Paramter Error " . $aConstants[$i] . " undefined");
	}
...
class CMCIC_Hmac {
	...
	//
	// Implémentation RFC 2104 HMAC pour PHP >= 4.3.0 - CrÈation d'un SHA1 HMAC.
...
//                           ********************
//              Encodage des caractères spÈciaux au format HTML
// ----------------------------------------------------------------------------
