...
class AuthorProtect {
	...
	public static function MakeContentAction( $skin, &$links ) {
		...
		if ( self::userIsAuthor( $title ) && $wgUser->isAllowed( 'authorprotect' ) && !$wgUser->isAllowed( 'protect' ) ) {
			...
			$links['actions']['authorprotect'] = array(
				...
				'text' => self::AuthorProtectMessage( $title ),
	...
	private static function AuthorProtectMakeProtectForm( $title ) {
		...
		$form = Html::rawElement( 'p', array(), htmlspecialchars( wfMsg( 'authorprotect-intro' ) ) );
		...
		$form .= $br . Xml::inputLabel( wfMsg( 'protectexpiry' ), 'wpExpiryTime', 'wpExpiryTime' );
		$form .= $br . Xml::inputLabel( wfMsg( 'protectcomment', 'reason' ), 'wpReason', 'wpReason' );
		$form .= $br . Html::hidden( 'wpToken', $token );
		$form .= $br . Xml::submitButton( wfMsg( 'authorprotect-confirm' ) );
