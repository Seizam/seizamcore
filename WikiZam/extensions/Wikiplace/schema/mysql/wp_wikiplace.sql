...
CREATE TABLE IF NOT EXISTS `wp_wikiplace` (
  ...
  `wpw_previous_total_page_hits` bigint(20) unsigned NOT NULL COMMENT 'Total page hits value when the current report started',
  `wpw_monthly_page_hits` bigint(20) unsigned NOT NULL COMMENT 'Number of monthly page hits',
  `wpw_previous_total_bandwidth` bigint(20) unsigned NOT NULL COMMENT 'Total bandwidth value when the current report started',
  `wpw_monthly_bandwidth` bigint(20) unsigned NOT NULL COMMENT 'FOR FUTUR USE: Total size in MB of monthly downloaded WikiPlace s files',
