...
class HTMLFormS {
    ...
    public function __construct($descriptor, /* IContextSource */ $context = null, $messagePrefix = '') {
        ...
        foreach ($descriptor as $fieldname => $info) {
            $section = isset($info['section']) ? $info['section'] : '';
    ...
    function getErrors($errors) {
        ...
        return $errorstr ? Html::rawElement('div', array('class' => 'informations error'), $errorstr) : '';
    }
     ...
     */
    function getTitle() {
        return $this->mTitle === false ? $this->getContext()->getTitle() : $this->mTitle;
    }
    ...
    public function getContext() {
        return $this->mContext instanceof IContextSource ? $this->mContext : RequestContext::getMain();
    }
...
//                }
        ...
        $classes = array('content_block');

        if (!$hasLeftColumn) { // Avoid strange spacing when no label exists
...
abstract class HTMLFormField {
    ...
    function getLabelHtml($cellAttributes = array()) {
        ...
        return Html::rawElement('label', $for, $this->getLabel());
    }
...
class HTMLTextAreaField extends HTMLFormField {
    ...
    function getCols() {
        return isset($this->mParams['cols']) ? $this->mParams['cols'] : 55;
    }

    function getRows() {
        return isset($this->mParams['rows']) ? $this->mParams['rows'] : 8;
    }
...
class HTMLCheckField extends HTMLFormField {
    ...
    function getInputHTML($value) {
        ...
        $checkbox = Xml::check($this->mName, $value, $attr) . '&#160;' .
     ...
     * For a checkbox, the label goes on the right hand side, and is
     * added in getInputHTML(), rather than HTMLFormField::getRow().
     * And we don't display any empty label.
     */
    function getLabelHtml($cellAttributes = array()) {
        return '';
    }
    ...
    function getInputHTML($value) {
        ...
        if ($value !== false) {
            $valInSelect = in_array(
                    $value, HTMLFormField::flattenOptions($this->mParams['options'])
            );
        }
    ...
    function formatOptions($options, $value) {
        ...
        foreach ($options as $label => $info) {
            if (is_array($info)) {
                ...
                $checkbox = Xml::check(
                                $this->mName . '[]', in_array($info, $value, true), $attribs + $thisAttribs);
                $checkbox .= '&#160;' . Html::rawElement('label', array('for' => "{$this->mID}-$info"), $label);

                $html .= ' ' . Html::rawElement('span', array('class' => 'mw-htmlform-multiselect-item'), $checkbox);
            }
        }
    ...
    function __construct($params) {
        ...
        if (array_key_exists('options', $params)) {
            ...
            $params['options'] = self::parseMessage(
                            wfMessage($params['options-message'])->inContentLanguage()->plain(), $params['other']
            );
        } else {
            # Sulk
    ...
    function formatOptions($options, $value) {
        ...
        foreach ($options as $label => $info) {
            if (is_array($info)) {
                ...
                $radiobox = Xml::radio(
                                $this->mName, $info, $info == $value, $attribs + array('id' => $id)
                );
                $radiobox .= '&#160;' .
                        Html::rawElement('label', array('for' => $id), $label);

                $html .= ' ' . Html::rawElement('span', array('class' => 'mw-htmlform-monoselect-item'), $radiobox);
            }
        }
...
class HTMLHiddenField extends HTMLFormField {
    ...
    public function getTableRow($value) {
        ...
        $this->mParent->addHiddenField(
                $this->mName, $this->mDefault, $params
        );
