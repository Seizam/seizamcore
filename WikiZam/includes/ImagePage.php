		...
		if ( $this->displayImg->exists() ) {
			...
			if ( $showLink ) {
				...
				if ( !$this->displayImg->isSafeFile() ) {
					...
					// See bug 25277.
					$out->addWikiText( <<<EOT
<div class="fullMedia"><span class="dangerousLink">{$medialink}</span> $dirmark<span class="fileInfo">$longDesc</span>{$mediafield}</div>
<div class="mediaWarning">$warning</div>
EOT
						);
				} else {
					$out->addWikiText( <<<EOT
<div class="fullMedia">{$medialink} {$dirmark}<span class="fileInfo">$longDesc</span>{$mediafield}
</div>
EOT
					);
				}
